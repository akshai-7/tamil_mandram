...
class ByLawController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            $Bylaw = Bylaw::query();
            $data = $Bylaw->where("org_id", Auth::user()->id)->orderBy("created_at", "desc")->get();
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    $btn  = '<a href="' . URL('by-law', ['id' => encrypt($row->id)])  . '" title="Edit" style="color:#004890;"  ><i class="fa fa-edit"></i></a>';
                    return $btn;
                })
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            Bylaw::create($request->except('_token', 'image', 'image_src',"image"));
            DB::commit();
            return redirect('by-law')->with(['success' => "By law created successfully..!"]);
        } catch (\Exception $e) {
